...
export class RoleResolver {
  ...
  /**
   * Ruling P4-R13: the caller's role read straight from the host, ignoring the cache window (and
   * refreshing it for later readers). Schema changes, grant and settings changes, and an app's
   * publish/rollback/archive take this path, so a demotion or removal cannot be outrun by up to
   * 30 seconds of cached membership. Reads and row writes stay on `resolveRole`: they run far
   * more often, and the same staleness there costs at most one stale read or write.
   */
  ...
  async assertAllowed(companyId: string, userId: string, allowedRoles: string[]): Promise<string> {
    ...
    if (!role || !allowedRoles.includes(role)) {
      throw new DataError("forbidden", `this operation is limited to company roles ${allowedRoles.join(", ")}`);
    }
    return role;
  }
